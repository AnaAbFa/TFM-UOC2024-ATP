...
data$round      <- relevel(data$round,      ref = "F") #Final
...
## Selection by BIC criteria using step function

step_1 <- step(glm_1, criterion = "BIC")
summary(step_1)
...
step_2 <- step(glm_2, criterion = "BIC")
summary(step_2)
...
step_3 <- step(glm_3, criterion = "BIC")
summary(step_3)
...
# The best model at the moment is step_3 (lowest BIC)
...
step_4 <- step(glm_4, criterion = "BIC")
summary(step_4)
...
step_5 <- step(glm_5, criterion = "BIC")
...
```
ANOVA proposes glm_4 as the best model. Since step_4 has a lower BIC and less terms, and therefore will be selected as the best fit for future comparisons (simplicity and interpretability over accuracy of the fit, when reasonable).
...
Tourney level used as random effect 
...
year used as random effect


```{r}

mixed_8 <- glmer(LLW ~ tourney_level + year + surface + 
                   dif_age + dif_rank + winner_hand + loser_hand + 
                   (1 | year),
                   family = binomial, offset = log(games), data = data_grouped_LLW)
summary(mixed_8)
testDispersion(mixed_8)
sum(residuals(mixed_8, type = "pearson")^2) / df.residual(mixed_7)



```
winner_id and loser_id cannit be used as a random effect because the number of random-effects parameters exceeds the number of observations, making the model unidentifiable and preventing reliable estimation of the random-effects variance components.
...
step_quadtratic <- step(quadratic_1, criterion = "BIC")
...
data_aggregated <- data_grouped_LLW %>%
  ...
  summarise(
    ...
    winner_hand = first(winner_hand),  # Select the first winner_hand 
    loser_hand = first(loser_hand),    # Select the first loser_hand
    dif_rank = mean(dif_rank, na.rm = TRUE),  # Take the mean of dif_rank
    dif_age = mean(dif_age, na.rm = TRUE),    #Take the mean of dif_age
    surface = first(surface),           # Select the first surface 
...
step_pois <- step(poisson_model, criterion = "BIC")
...
step_pois2 <- step(poisson_model_2, criterion = "BIC")
...
step_nbr <- step(nbr_1, criterion = "BIC")
summary(step_nbr)
...
new_data_grouped_LL$rank_category <- factor(new_data_grouped_LL$rank_category)
new_data_grouped_LL$rank_category  <- relevel(new_data_grouped_LL$rank_category, ref = "Medium")
...
data_aggregated_2 <- new_data_grouped_LL %>%
  ...
  summarise(
    ...
    loser_hand = first(loser_hand),    # Select the first loser_hand
    dif_rank = mean(dif_rank, na.rm = TRUE),  # Take the mean of dif_rank 
    rank_category = first(rank_category), # Select the first category
    age_category = first(age_category), # Select the first category
    dif_age = mean(dif_age, na.rm = TRUE),    # Take the mean of dif_age
    surface = first(surface),           # Select the first surface
    year = first(year)                  # Select the first year
  )
...
step_pois3 <- step(poisson_model_3, criterion = "BIC")
summary(step_pois3)
...
step_6 <- step(glm_6, criterion = "BIC")
summary(step_6)
...
step_12 <- step(glm_12, criterion = "BIC")
summary(step_12)
...
complete_data_LL_validation <- incomplete_data_LL
  
  #incomplete_data_LL[complete.cases(incomplete_data_LL[, c("tourney_level", "rank_category")]), ]
...
pred_probs <- predict(step_6, newdata = complete_data_LL_validation, type = "response")
pred_classes <- ifelse(pred_probs > 0.5, 1, 0)

# Confusion matrix and related metrics
conf_matrix <- confusionMatrix(as.factor(pred_classes), as.factor(complete_data_LL_validation$LLW))
conf_matrix
...
emm_level <- emmeans(step_6, ~tourney_level, type = 'response')
emm_rank <- emmeans(step_6, ~rank_category, type = 'response')
